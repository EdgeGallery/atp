...
/**
 * app memory usage validation.
 */
public class MemUsageValidation {

    private static final Logger LOGGER = LoggerFactory.getLogger(MemUsageValidation.class);

    private static final String INTERFACE_RESPONSE_FAILED = "get kpi info from appo response failed, the status is: %s";

    private static final String MEM_USAGE_TOO_HIGH = "app memory usage is more than 50%.";
    ...
    public String execute(String filePath, Map<String, String> context) {
        ...
        try {
            ...
            LOGGER.info("mem response: {}", jsonObject.toString());
            JsonObject cpuUsage = jsonObject.get("memusage").getAsJsonObject();
            int used = cpuUsage.get("used").getAsInt();
            int total = cpuUsage.get("total").getAsInt();
            int usedBeforeDeploy = Integer.valueOf(memUsedBeforeDeploy);
            return ((used - usedBeforeDeploy) / total) > 0.5 ? MEM_USAGE_TOO_HIGH : SUCCESS;
