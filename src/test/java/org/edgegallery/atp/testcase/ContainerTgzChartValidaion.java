...
/**
 * validate existence of chart.yaml in tgz fille.
 */
public class ContainerTgzChartValidaion {
    private static final String INNER_EXCEPTION = "inner exception, please check the log.";

    private static final String TGZ_NOT_EXISTS = "there is no .tgz file in Artifacts/Deployment/Charts dir";

    private static final String CHART_NOT_EXISTS_IN_TGZ
        = "there is no chart.yaml file in .tgz file of Artifacts/Deployment/Charts dir";
        ...
        try (ZipFile zipFile = new ZipFile(filePath)) {
            ...
            while (entries.hasMoreElements()) {
                ...
                if (entry.getName().startsWith("Artifacts/Deployment/Charts") && entry.getName().endsWith(".tgz")) {
                    return null == analysizeTgz(zipFile, entry) ? CHART_NOT_EXISTS_IN_TGZ : SUCCESS;
                }
            }
        } catch (IOException e) {
    ...
    private String analysizeTgz(ZipFile zipFile, ZipEntry entry) {
        try (TarArchiveInputStream tarIn = new TarArchiveInputStream(
            ...
            while ((entryTar = tarIn.getNextTarEntry()) != null) {
                ...
                if ("chart.yaml".equalsIgnoreCase(nameArray[nameArray.length - 1])) {
                    return SUCCESS;
                }
            }
        } catch (IOException e) {
