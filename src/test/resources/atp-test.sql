    ...
    INSERT INTO public.test_scenario_table(
      ...
      ('e71718a5-864a-49e5-855a-5805a5e9f97d','中国联通','China Unicom Scenario','适用于中国联通场景的测试','suite for China Unicom test','China Unicom', now()::timestamp without time zone),
      ('6fe8581c-b83f-40c2-8f5b-505478f9e30b','中国移动','China Mobile Scenario','适用于中国移动场景的测试','suite for China Mobile test','China Mobile', now()::timestamp without time zone),
      ('96a82e85-d40d-4ce5-beec-2dd1c9a3d41d','中国电信','China Telecom Scenario','适用于中国电信场景的测试','suite for China Telecom test','China Telecom', now()::timestamp without time zone);
    ...
    INSERT INTO public.test_suite_table(
      ...
      ('111684bd-d6df-4b47-aab8-b43f1b4c19c0','中国移动遵从性测试','China Mobile Compliance Test','遵从中国移动标准对应用包结构进行校验','Validate app package structure according to China Mobile standard','6fe8581c-b83f-40c2-8f5b-505478f9e30b', now()::timestamp without time zone),
      ('1114da1b-1f36-4295-920a-8074f7f9d942','中国移动沙箱测试','China Mobile Sandbox Test','应用包部署测试中国移动版','App package deployment test China Mobile version','6fe8581c-b83f-40c2-8f5b-505478f9e30b', now()::timestamp without time zone),
      ('222684bd-d6df-4b47-aab8-b43f1b4c19c0','中国联通遵从性测试','China Unicom Compliance Test','遵从中国联通标准对应用包结构进行校验','Validate app package structure according to China Unicom standard','e71718a5-864a-49e5-855a-5805a5e9f97d', now()::timestamp without time zone),
      ('2224da1b-1f36-4295-920a-8074f7f9d942','中国联通安全性测试','China Unicom Security Test','安全性测试中国联通版','App package security test China Unicom version','e71718a5-864a-49e5-855a-5805a5e9f97d', now()::timestamp without time zone),
      ('333684bd-d6df-4b47-aab8-b43f1b4c19c0','中国电信遵从性测试','China Telecom Compliance Test','遵从中国电信标准对应用包结构进行校验','Validate app package structure according to China Telecom standard','96a82e85-d40d-4ce5-beec-2dd1c9a3d41d', now()::timestamp without time zone),
      ('3334da1b-1f36-4295-920a-8074f7f9d942','中国电信沙箱测试','China Telecom Sandbox Test','应用包部署测试中国电信版','App package deployment test China Telecom version','96a82e85-d40d-4ce5-beec-2dd1c9a3d41d', now()::timestamp without time zone);
    ...
    INSERT INTO public.test_case_table(
     id, nameCh,nameEn, hashCode,type, classname,  descriptionCh,descriptionEn,filePath,codeLanguage,expectResultCh,expectResultEn,testStepCh,testStepEn,testSuiteIdList,createTime)
      ...
      ('4d203173-1011-4f62-aabb-8ebcec357f87','中国联通MEAD文件目录校验','China Unicom MEAD File Dir Validation','','automatic','MEADFileDirValidation','MEAD文件在根目录的存在性校验','MEAD file dir existence in root path Validation','','java','应用包根目录下必须存在MEAD文件夹','There must exist MEAD file dir in app package root path','1.解压应用包 2.检查根目录是否存在MEAD文件夹','1. unzip app package 2. check the existence of MEAD file in root path','222684bd-d6df-4b47-aab8-b43f1b4c19c0', now()::timestamp without time zone),
      ('4d203173-1012-4f62-aabb-8ebcec357f87','中国联通MF文件必填字段校验','China Unicom MF Field Validation','','automatic','CUMFFieldValidation','MF文件必填字段校验','MF file required field validation','','java','MF文件必须包含如下字段：app_name,app_provider,app_package_version,app_release_data_time,app_type or app_package_description','MF file must contain the following fields: app_name,app_provider,app_package_version,app_release_data_time,app_type or app_package_description','1.解压应用包 2.解析mf文件是否包含必填字段','1. unzip app package 2. validate the required fields in mf file','222684bd-d6df-4b47-aab8-b43f1b4c19c0', now()::timestamp without time zone),
      ('4d203173-1013-4f62-aabb-8ebcec357f87','中国联通Swagger文件目录校验','China Unicom Swagger File Dir Validation','','automatic','SwaggerFileDirValidation','Swagger文件在根目录的存在性校验','Swagger file dir existence in root path Validation','','java','应用包根目录下必须存在Swagger文件夹','There must exist Swagger file dir in app package root path','1.解压应用包 2.检查根目录是否存在Swagger文件夹','1. unzip app package 2. check the existence of Swagger file in root path','222684bd-d6df-4b47-aab8-b43f1b4c19c0', now()::timestamp without time zone),
     ('4d203173-1014-4f62-aabb-8ebcec357f87','中国联通MF文件Hash值校验','China Unicom MF File Hash Value Validation','','automatic','MFHashValueValidation','MF文件Hash值正确性校验','MF file hash value validation','','jar','MF文件中每个Source对应的文件的Hash值必须是正确的','Each hash value of Source field file must be right','1.解压应用包 2.查看Source文件对应的hash值是否正确','1. unzip app package 2. validate the hash of Source file is right','2224da1b-1f36-4295-920a-8074f7f9d942', now()::timestamp without time zone);
